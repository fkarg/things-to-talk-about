\documentclass[12pt,compress,english,utf8,t,usenames,dvipsnames,handout]{beamer}
\usepackage[ngerman, english,main=english]{babel}


\title{Hierarchical Temporal Memory}
\subtitle{Biological And Machine Intelligence}
\author{Felix Karg}


\graphicspath{ {../template/} {./graphics/temporal/} {./graphics/} {../template_tex/} } % add further graphics paths here


\input{setup}
% \input{setup}



\newif\ifonline
\onlinefalse
% \onlinefalse


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%BEGINNING%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % \input{chapters/introduction}
\input{chapters/note}
\input{chapters/bio}
\input{chapters/overview}

\section{Core Concepts}
\input{chapters/hierarchies}
\input{chapters/sdr}

\input{chapters/learning}
\input{chapters/spatial_pooling}
\input{chapters/temporal_pooling}

\input{chapters/recap}
% \input{chapters/grid_cells}
\input{chapters/implications}


% \input{chapters/open}



% \input{chapters/ml}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%SOURCES%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Sources}
\begin{frame}[c,fragile,allowframebreaks]{Sources}
    \Large
    \vfill
The slides are online: \url{https://git.io/JeYPO} \vfill
Drop me a mail: fkarg10@gmail.com \vfill 
Content mainly from \cite{hawkins2017book}. \vfill
    \newpage
% \bibliographystyle{plainnat}
\bibliographystyle{ieeetr}
\bibliography{references.bib}
\end{frame}


\input{chapters/additional}



\end{document}


% TODO:

% Include Topics/Facts:
%
% - explain reticular activating system
% - explain gradient for Autism and that it is more 'bottom up'
% - explain 'top-down' phenomena and common biases
% - explain theory: inhibited-boost: hallucination

% Roadmap (?)
%
% - Introduce HTMv3 !
% - Motor movements are just very strong predictions the body 'makes happen'
%   - Flow state is where everything is exactly as predicted
% - Mistakes that happen by accidential correlation: search for good example; mistaking something to be true
% - explain gradient for Einstein/Craziness, proving vs disproving in the brain -> THC is bad
% - Mistakes that happen by top-down enforcing (e.g. the dalmatine picture, scrambled words)
% - Location-based composition: This explains mind palaces!
% - Everything is a 'conceptspace' -

% - Explain Priming based on predictive stuff (the priming-bias)
% - Explain biases based on these models
% - Explain the fact that smarter people have less Brain activity
% - Explain the bayesian-reasioning part
% - Explain how cortical columns create quite powerful models
% - Phantom limb 'pain' -> boosting
% - AHA-Moment (aligning models, ceasing bursting)
% - requiring more resources during learning



% Main Feedback:
% 
% Detailed introduction, but bring the dots together earlier.
% Have more of a 'red line' through all of this.
%   -> maybe kind of start with HTM pipeline and take it from there
%   -> for technical audience, less explanation for combinatorics/sdr is needed
%   -> maybe gloss over a lot of things in the beginning, start with results
%       and work back from implications, how they actually work
%       (after having explained the basic pipeline)
% TODO: put this feedback in v2 of this presentation.




