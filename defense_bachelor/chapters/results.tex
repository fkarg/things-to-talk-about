...
\begin{frame}[c]{Peak Memory Usage}
    \only<1>{\includegraphics[scale=0.8]{maxresident_50}}
    \only<2>{\includegraphics[scale=0.8]{maxresident_25}}
\end{frame}
...
\begin{frame}[c]{Loading Times}
    \only<2>{\includegraphics[scale=0.8]{loadtimes_25}}
    \only<3>{\includegraphics[scale=0.8]{loadtimes_50}}
\end{frame}


\begin{frame}[c]{Runtime Length}
    \only<2>{\includegraphics[scale=0.8]{runtime_50}}
    \only<3>{\includegraphics[scale=0.8]{runtime_25}}
\end{frame}


\begin{frame}[c]{Multicore Runtime Length Comparison}
    \only<2>{\includegraphics[scale=0.35]{correction_process}}
    \only<3>{\includegraphics[scale=0.8]{compute_multi_25}}
    \only<4>{\includegraphics[scale=0.8]{runtime_multi_50}}
    \only<5>{\includegraphics[scale=0.8]{runtime_multi_25}}
\end{frame}


\begin{frame}[c]{Multicore Memory Comparison}
    \only<2>{\includegraphics[scale=0.8]{maxresident_multi_25}}
    \only<3>{\includegraphics[scale=0.8]{memiter_multi_25}}
\end{frame}
